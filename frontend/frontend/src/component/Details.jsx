...
const Details = () => {
  ...
  useEffect(() => {
    const fetchUserandCoachDetails = async () => {
      try {
        const response = await axios.get("http://localhost:4568/user/all");
        const response1 = await axios.get("http://localhost:4568/coach/all");
  ...
  const handleUserDelete = async (userId) => {
    try {
      await axios.delete(`http://localhost:4568/user/delete/${userId}`);
  ...
  const handleCoachDelete = async (coachId) => {
    try {
      await axios.delete(`http://localhost:4568/coach/delete/${coachId}`);
  ...
  const handleSaveUser = async () => {
    try {
      await axios.put(`http://localhost:4568/user/update/${editingUser.id}`, editingUser);
      setEditingUser(null);
  ...
  const handleSaveCoach = async () => {
    try {
      await axios.put(`http://localhost:4568/coach/update/${editingCoach.id}`, editingCoach);
      setEditingCoach(null);
      // Refresh the coach list after saving
      const response1 = await axios.get("http://localhost:4568/coach/all");
      setCoaches(response1.data);
    } catch (error) {
  ...
  const handleAdminApprovalforUser = async (userId) => {
    try {
      const response = await axios.put(`http://localhost:4568/approve/user/${userId}`);
      ...
      // Handle success, show a notification, etc.
  ...
  const handleAdminApprovalforCoach = async (coachId) => {
    try {
      const response = await axios.put(`http://localhost:4568/approve/coach/${coachId}`);
