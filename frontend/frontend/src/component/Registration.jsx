...
export function Registration() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.role === 'user') {
        response = await axios.post("http://localhost:4568/user/register", formData);
      } else if (formData.role === 'coach') {
        response = await axios.post("http://localhost:4568/coach/register", formData);
      }
  ...
  const handleRoleChange = (value) => {
    setFormData((prevFormData) => {
      ...
      // Reset additional fields based on the selected role
      if (value === 'user') {
