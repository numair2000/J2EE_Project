...
export const AddEvent = () => {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        if (eventId) {
          const response = await axios.get(`http://localhost:4568/${eventId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (eventId) {
        await axios.put(`http://localhost:4568/update/${eventId}`, eventData);
        window.alert('Event updated successfully!');
      } else {
        await axios.post("http://localhost:4568/create", eventData);
